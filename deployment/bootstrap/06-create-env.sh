#!/usr/bin/env bash
# 06-create-env.sh — generate secrets, create the one missing env file this
# DEPLOY_ENV actually uses (.env.development or .env.production — see
# env_file_for in lib/common.sh), and validate the result. Delegates to the
# same ensure_env_files/env-check.sh install.sh itself uses — no separate
# implementation.
#
